...
with col3:
    total_demand = sum(v.get("总需求", 0) if isinstance(v, dict) else v for v in demands.values()) if isinstance(demands, dict) else 0
...
if st.button("🚀 开始优化计算", type="primary", use_container_width=True):
    ...
    try:
        ...
        report_lines.append(f"  总物资需求: {sum(v.get('总需求', 0) if isinstance(v, dict) else v for v in demands.values()) if isinstance(demands, dict) else 0:.0f} kg")
