...
5. **路径优化** - 执行物流网络优化计算（K-Means选址 + OR-Tools FSMVRP）
6. **碳排放概览** - 赛事物流碳足迹实时监控
...
1. VRP路径优化
采用 OR-Tools 异构车队求解（FSMVRP，车队规模与组成问题）。
用户只需勾选可用动力类型并填写每类车的**实际载重**与**可用上限**，算法会在不超过上限的前提下自行决定实际派车数与路线。
碳排放计算采用逐站载重递减公式：`E = Σ(距离 × 碳因子 × 当前载重)`（碳因子来自车型库默认值）。
