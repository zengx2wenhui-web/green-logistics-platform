...
## v1.1 — 全局工作流引导 + 场馆图注 + 物资智能导入（2026-04-13）

本次修改涉及 **9 个文件**，核心目标是增加页面间流转引导、增强场馆地图可视化、重构物资需求导入逻辑。

### 一、全局工作流引导（模块一）

**涉及文件：** `app.py`、`pages/1_仓库设置.py` ~ `pages/8_优化结果.py`（全部 9 个页面）

- 在每个页面底部添加「下一步：XXX ➡️」按钮，使用 `st.switch_page()` 原生跳转
- 最后一页（优化结果）添加「返回首页 🏠」按钮
- Session State 数据在页面切换时完整保留

### 二、场馆录入 — 地图图注与颜色映射（模块二）

**涉及文件：** `pages/2_场馆录入.py`

- 定义全局颜色映射字典 `VENUE_TYPE_COLORS`，同时包含 Folium Icon 颜色名和 CSS hex 值
- 场馆类型名称更新：「比赛场馆」→「比赛场地」，「训练场馆」→「训练场地」
- 添加向下兼容映射 `_TYPE_COMPAT`，自动转换旧数据的类型名
- 在 Folium 地图左下角添加悬浮 HTML 图注（Legend）：8 种场馆类型 × 颜色方块
- 图注颜色与 Marker 颜色严格一一对应

### 三、物资需求 — 动态导入与智能清洗（模块三，核心重构）

**涉及文件：** `pages/3_物资需求.py`

- **取消前端手动列映射**：删除 `st.selectbox` 列映射组件，改为后端静默处理
- **智能表头定位** `_smart_read_excel()`：扫描前 20 行，以包含「场馆名称」关键字的行作为 DataFrame 真实表头，跳过「十五运会汇总表」等无关大标题
- **动态列识别** `_is_material_col()`：锁定「场馆名称」列，自动提取除序号/备注/合计/总计/Total 以外的物资分项列
- **合计排雷**：过滤关键词集合 `_EXCLUDE_COL_KEYWORDS = {"合计", "总计", "总需求", "total", "小计", "subtotal", "sum"}`
- **智能单位换算** `_parse_weight_to_kg()`：正则匹配 `500g`、`2t`、`1.5吨`、`3千克`、`200斤` 等带单位字符串，统一换算为 kg 浮点数
- **重计算总需求**：忽略原表旧合计列，对清洗后分项按行重新 `sum()`
- **精简输出**：最终展示三列 — 序号、场馆名称、总需求(kg)

### 四、文档更新

- `README.md`：补充智能物资导入、全局工作流引导、地图图注等新功能说明
- `CHANGELOG.md`：新增 v1.1 修改记录

---

## v1.0 — 初始重构（OR-Tools 移除 + Bug 修复）
